...
import models.{ConfirmVendorAddress, NormalMode, UserAnswers}
...
class ConfirmVendorAddressControllerSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  val formProvider = new ConfirmVendorAddressFormProvider()
  val form = formProvider()

  "ConfirmVendorAddress Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ConfirmVendorAddressPage, ConfirmVendorAddress.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(ConfirmVendorAddress.values.head), NormalMode)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, confirmVendorAddressRoute)
